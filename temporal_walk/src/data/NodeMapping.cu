...
void NodeMapping::update(const EdgeData& edges, const size_t start_idx, const size_t end_idx) {
    ...
    if (use_gpu) {
        ...
        max_node_id = std::max(*max_src, *max_tgt);
        #endif
    } else {
    ...
    if (use_gpu) {
        #ifdef HAS_CUDA
        for (size_t i = start_idx; i < end_idx; i++) {
            const int src = edges.sources[i];
            const int tgt = edges.targets[i];

            is_deleted[src] = ITEM_NOT_DELETED;
            is_deleted[tgt] = ITEM_NOT_DELETED;
