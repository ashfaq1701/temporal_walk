...
protected:
    ...
    {
        auto host_offsets = group_offsets.to_vector();
        auto host_weights = weights.to_vector();

        // For each node
        for (size_t node = 0; node < host_offsets.size() - 1; node++) {
            ...
            if (start < end) {
                ...
                EXPECT_NEAR(host_weights[end-1], 1.0, 1e-6);
            }
        }
    }
...
TEST_F(NodeEdgeIndexWeightTest, TimescaleNormalizationTest) {
    ...
    const auto host_offsets = index.outbound_timestamp_group_offsets.to_vector();
    ...
    const auto host_timestamps = edges.unique_timestamps.to_vector();
