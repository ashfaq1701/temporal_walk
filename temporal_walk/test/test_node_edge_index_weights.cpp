...
{
protected:
    static void verify_node_weights(const std::vector<size_t>& group_offsets, const std::vector<double>& weights,
                                    const bool forward)
    {
        // For each node
...
TEST_F(NodeEdgeIndexWeightTest, WeightBiasPerNode)
{
    ...
    auto get_individual_weights = [](const std::vector<double>& all_weights,
                                     ...
                                     size_t node)
    {
        ...
        weights.push_back(all_weights[start]);
        return weights;
    };
    ...
    {
        const double expected_ratio = exp(-100); // Time diff between groups is 100
...
TEST_F(NodeEdgeIndexWeightTest, TimescaleNormalizationTest)
{
    ...
    // Helper to get individual weights from all weights
    auto get_weights = [](const std::vector<double>& all_weights, size_t start, size_t end)
    {
        std::vector<double> weights;
        weights.push_back(all_weights[start]);
        for (size_t i = start; i < end; i++)
        {
            weights.push_back(all_weights[i]);
        }
        return weights;
    };
