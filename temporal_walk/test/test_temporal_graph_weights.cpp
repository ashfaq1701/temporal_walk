...
protected:
    ...
    static void verify_cumulative_weights(const DualVector<double>& weights) {
        ...
        ASSERT_FALSE(host_weights.empty());
...
TEST_P(TemporalGraphWeightTest, NodeWeightComputation) {
    ...
    const auto host_offsets = node_index.outbound_timestamp_group_offsets.to_vector();
    ...
    auto host_weights = node_index.outbound_forward_cumulative_weights_exponential.to_vector();
...
TEST_P(TemporalGraphWeightTest, WeightScalingPrecision) {
    ...
    // Get node's group range
    const auto host_offsets = graph.node_index.outbound_timestamp_group_offsets.to_vector();
