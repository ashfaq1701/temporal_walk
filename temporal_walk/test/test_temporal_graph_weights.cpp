...
TEST_F(TemporalGraphWeightTest, EdgeWeightComputation) {
    ...
    const auto& fwd_weights = graph.edges.forward_cumulative_weights_exponential;
    const auto& bwd_weights = graph.edges.backward_cumulative_weights_exponential;
...
TEST_F(TemporalGraphWeightTest, NodeWeightComputation) {
    ...
    std::vector<double> node_out_weights(
        node_index.outbound_forward_cumulative_weights_exponential.begin() + static_cast<int>(start_pos),
        node_index.outbound_forward_cumulative_weights_exponential.begin() + static_cast<int>(end_pos));
...
TEST_F(TemporalGraphWeightTest, WeightScalingPrecision) {
    ...
    const auto& edge_weights = graph.edges.forward_cumulative_weights_exponential;
    ...
    const auto& node_weights = graph.node_index.outbound_forward_cumulative_weights_exponential;
